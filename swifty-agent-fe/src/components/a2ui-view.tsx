...
// Mirrors the Go backend constant (internal/ai/a2ui UIActionPrefix): the
// prompt teaches the LLM to treat messages with this prefix as UI actions.
const UI_ACTION_PREFIX = "[UI_ACTION]";
  ...
  const processor = useMemo(
    () =>
      new MessageProcessor<ReactComponentImplementation>(
        [shadcnCatalog],
        // eslint-disable-next-line react-hooks/refs -- the ref is only read when an action fires, never during render
        (action) => {
